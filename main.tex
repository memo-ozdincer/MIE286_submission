...
{\small Team 51 \hfill April 7, 2026}
\end{center}
...
The experiment interface was a browser-based application hosted on GitHub Pages. Targets (radius = 35\,px) appeared at random canvas positions and participants were instructed to click them as quickly and accurately as possible. Timing was recorded via the browser's \texttt{performance.now()} API (sub-millisecond precision). A 400\,ms inter-trial interval separated targets. Figure~\ref{fig:feedback_states} shows the three visual feedback states.
...
There were two levels of feedback for the experiment, included as a within-subject factor:
...
Both conditions conveyed identical three-result information. The on-screen HUD (trial counter, score, streak) was identical in both conditions. This addresses proposal feedback, which noted an information asymmetry in the original design.
...
Instructions $\;\to\;$ 5 practice trials $\;\to\;$ \textbf{Block 1} (30 or 60 trials) $\;\to\;$ Break $\;\to\;$ \textbf{Block 2} $\;\to\;$ End screen
}}
\vspace{4pt}

Condition order and target positions were randomized for each participant. Gamification with scoring, graphic UI components, and a leaderboard was implemented to boost user engagement.

\subsection{Potential Biases}

The convenience sampling limits generalizability. Device variability was controlled as a covariate in the H\textsubscript{3} regression. Audio environment was uncontrolled. Four participants completed the experiment twice, as a result each attempt was treated as a separate observation.
...
Only participants who completed the full 60-trial protocol were included ($N = 41$ of 72 total). Trials with RT $< 150$\,ms or $> 8{,}000$\,ms were excluded. The remaining 4{,}919 trials were aggregated to per-participant means for each condition, yielding $N = 41$ paired observations.

\subsection{Assumption Checks}

Normality of within-subject differences was assessed with the Shapiro--Wilk test [9] and Q-Q plots (Appendix A). Reaction-time differences ($W = 0.970$, $p = .336$) and error differences ($W = 0.972$, $p = .406$) were approximately normal, supporting the use of paired $t$-tests were appropriate for both H\textsubscript{1} and H\textsubscript{2}.

\subsection{Hypothesis Tests}

\noindent\textbf{H\textsubscript{1} (Speed).} Our original prediction, based on MRT [1,2], was that auditory feedback would be faster. The data showed the opposite. We used a \emph{two-tailed} paired $t$-test because our directional prediction was reversed:
\vspace{-4pt}
\begin{align*}
  H_0 &: \mu_{\text{vis}} = \mu_{\text{aud}} &\quad H_1 &: \mu_{\text{vis}} \neq \mu_{\text{aud}}
\end{align*}

\noindent\textbf{H\textsubscript{2} (Accuracy).} One-tailed paired $t$-test where $d = \bar{e}_{\text{vis}} - \bar{e}_{\text{aud}}$ per participant:
\vspace{-4pt}
\begin{align*}
  H_0 &: \text{median}(d_{\text{error}}) \leq 0 &\quad H_1 &: \text{median}(d_{\text{error}}) > 0
\end{align*} 
\textbf{H\textsubscript{3} (Speed-Accuracy Tradeoff).} Multiple linear regression with interaction:
\[
  \text{error} = \beta_0 + \beta_1 \!\cdot\! \text{Reaction Time} + \beta_2 \!\cdot\! \text{condition} + \beta_3 \!\cdot\! (\text{Reaction Time} \times \text{condition}) + \varepsilon
\]
A significant $\beta_3$ indicates different Speed-Accuracy Tradeoff slopes. Supplemented with Pearson correlations and simple linear regression per condition.
...
Table~\ref{tab:dvs} summarizes the two dependent variables. Visual feedback was associated with significantly faster responses ($-18.5$\,ms, $p = .047$) and significantly greater error ($+1.0$\,px, $p = .045$), consistent with a speed-accuracy tradeoff.
...
Error (px) & 13.9 & 5.6 & 12.9 & 4.5 & $+1.0$ & .045 \\
...
\textbf{H\textsubscript{2} result.} Paired $t$-test: $t(40) = 1.74$, $p = .045$ (one-tailed), $d = 0.27$. Significant: visual feedback produced greater radial error, consistent with a speed-accuracy tradeoff.
...
\begin{enumerate}[label={[\arabic*]},leftmargin=1.5em,itemsep=2pt]
  ...
  \item R.\,E.\ Walpole, R.\,H.\ Myers, S.\,L.\ Myers, K.\ Ye, \emph{Probability \& Statistics for Engineers \& Scientists}, 9th\,ed., Prentice Hall, 2011.
\end{enumerate}
}
...
\begin{itemize}
  \item RT differences: $W = 0.970$, $p = .336$ : normal. Paired $t$-test appropriate.
  \item Error differences: $W = 0.972$, $p = .406$ : approximately normal. Paired $t$-test appropriate.
\end{itemize}
...
\begin{subfigure}[t]{0.48\textwidth}
  ...
  \caption{Error differences: approximately normal.}
\end{subfigure}
...
\begin{enumerate}
  ...
  \item \texttt{04\_inferential.R} : Paired $t$-tests (H\textsubscript{1}, H\textsubscript{2}), multiple regression with interaction (H\textsubscript{3}).
...
Full code, data, and figures are available in the reproducibility repository: \url{https://github.com/memo-ozdincer/MIE286_submission}
...
H\textsubscript{2}: Error & $V = 203$ & $.811$ & $t = 1.74$ & $.045$* \\
